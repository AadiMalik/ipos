$(document).ready(function () {
    ...
    if ($('#c_date_filter').length == 1) {
        $('#c_date_filter').daterangepicker(dateRangeSettings, function (start, end) {
            $('#c_date_filter').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );
            customer_report_tbl.ajax.reload();
        });
        $('#c_date_filter').on('cancel.daterangepicker', function (ev, picker) {
            $('#c_date_filter').val('');
            customer_report_tbl.ajax.reload();
        });
    }
    ...
    customer_report_tbl = $('#customer_report_tbl').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                d.location_id = $('#c_report_location_id').val();
                var start = $('input#c_date_filter')
                    .data('daterangepicker')
                    .startDate.format('YYYY-MM-DD');
                var end = $('input#c_date_filter')
                    .data('daterangepicker')
                    .endDate.format('YYYY-MM-DD');
                d.start_date = start;
                d.end_date = end;
                d.contact_id = $('#c_contact_id').val();
            }
        },
        columnDefs: [
            { targets: [5], orderable: false, searchable: false },
        ...
        columns: [
            ...
            { data: 'passport_picture', name: 'passport_picture' },
            { data: 'pep_type', name: 'pep_type' },
    ...
    if ($('table#product_sell_report_table').length == 1) {
        ...
        product_sell_report = $('table#product_sell_report_table').DataTable({
            ...
            columns: [
                ...
                { data: 'subtotal', name: 'subtotal', searchable: false },
                { data: 'returntotal', name: 'returntotal', searchable: false },
                { data: 'nettotal', name: 'nettotal', searchable: false },
            ...
            fnDrawCallback: function (oSettings) {
                ...
                $('#footer_returntotal').text(
                    sum_table_col($('#product_sell_report_table'), 'row_returntotal')
                );
                $('#footer_nettotal').text(
                    sum_table_col($('#product_sell_report_table'), 'row_nettotal')
                );
    ...
    //Sell Payment Report
    agent_commission_report = $('table#agent_commission_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[2, 'desc']],
        ajax: {
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'customer', orderable: false, searchable: false },
            { data: 'location_name', orderable: false, searchable: false },
            { data: 'pep_type', orderable: false, searchable: false },
            { data: 'risk_rating', orderable: false, searchable: false },
    ...
    $('#agent_commission_report_form #location_id').change(
        function () {
            agent_commission_report.ajax.reload();
        }
    );
    if ($('#acr_date_filter').length == 1) {
        $('#acr_date_filter').daterangepicker(dateRangeSettings, function (start, end) {
            $('#acr_date_filter span').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );
            agent_commission_report.ajax.reload();
        });
        $('#acr_date_filter').on('cancel.daterangepicker', function (ev, picker) {
            $('#acr_date_filter').val('');
            agent_commission_report.ajax.reload();
        });
    }
