...
$(document).ready(function () {
    ...
    $('button#posEditDiscountModalUpdate').click(function () {
        ...
        __write_number($('input#discount_amount'), final_discount);

        // Handle reward points if enabled
        if ($('#reward_point_enabled').length) {
...
function pos_discount(total_amount) {
    ...
    // ✅ 1.15 se divide
    var adjusted_discount = calculation_amount;

    var discount = __calculate_amount(calculation_type, adjusted_discount, total_amount);
...
function validate_discount_field() {
    ...
    if (discount_element.val() != '' && !isNaN(max_value)) {
        ...
        // ✅ custom check for 1.15 division
