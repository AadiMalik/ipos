...
class ProductUtil extends Util
{
    ...
    public function getProductValuationDetails($business_id, $filters)
    {
        $query = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ->leftJoin('categories as c', 'p.category_id', '=', 'c.id')
            ->leftJoin('variation_location_details as vld', 'variations.id', '=', 'vld.variation_id')
        ...
        $products = $query->select(
            ...
            // Purchase Price (avg based on purchases in date range)
            DB::raw("(SELECT AVG(pl.purchase_price_inc_tax) 
                  FROM purchase_lines pl 
                  JOIN transactions t ON pl.transaction_id=t.id
                  WHERE pl.variation_id=variations.id $date_filter) as purchase_price"),

            // Profit = sale price - avg purchase price
            DB::raw("(
            variations.sell_price_inc_tax - 
            (SELECT AVG(pl.purchase_price_inc_tax) 
             FROM purchase_lines pl 
             JOIN transactions t ON pl.transaction_id=t.id
             WHERE pl.variation_id=variations.id $date_filter)
        ) as profit")
    ...
    public function getProductStockMovementReport($business_id, $filters)
    {
        $query = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ->leftJoin('categories as c', 'p.category_id', '=', 'c.id')
            ->leftJoin('variation_location_details as vld', 'variations.id', '=', 'vld.variation_id')
        ...
        if ($start_date) {
            ...
            $date_condition_purchase = "AND t.transaction_date BETWEEN '$start_date' AND '$end_date'";
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        $stock_history = Transaction::leftjoin(
            ...
            ->leftjoin(
                'purchase_lines as rpl',
                'rpl.transaction_id',
                '=',
                'return.id'
            )
            ->leftjoin(
                'transaction_sell_lines as rsl',
                'rsl.transaction_id',
                '=',
                'return.id'
            )
