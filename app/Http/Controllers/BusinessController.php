    ...
    public function testWhatsAppConfiguration(Request $request)
    {
        try {
            ...
            if (empty($instance_id) || empty($api_token)) {
                throw new \Exception("Instance ID & API Token are required.");
            }

            // Test by fetching settings from Green API
            $url = "https://api.green-api.com/waInstance{$instance_id}/getSettings/{$api_token}";

            $response = Http::get($url);

            if (!$response->successful()) {
                throw new \Exception("Green API Error: " . $response->body());
            }

            $data = $response->json();

            if (empty($data['wid'])) {
                throw new \Exception("Unable to verify WhatsApp connection.");
            }

            $output = [
                'success' => 1,
                'msg' => "WhatsApp API connected successfully!",
            ];
        } catch (\Exception $e) {
            \Log::emergency('File: ' . $e->getFile() . ' Line: ' . $e->getLine() . ' Message: ' . $e->getMessage());
